...
这是一套适合新手练手的纯前端的数据标注实训系统原型，目标是先把“完整项目”的页面、角色、流程和交互跑通。它适合课程设计、毕业设计早期原型、需求演示，也适合作为后续接入后端和数据库的基础。
